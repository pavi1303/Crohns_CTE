    ...
    for pset = 1:length(pruning_stage)
        ...
        for iter=1:100
            ...
            for fset=1:3
                ...
                [training_set,mean_val1,mad_val1]=simplewhiten(training_set,mean_val,mad_val);
                ...
                [temp_stats1_sw,~] = Classify_wrapper('RANDOMFOREST', training_set(:,fea) , testing_set1(:,fea), training_labels(:), ...
                    testing_labels(:), options);
                [temp_stats2_sw,~] = Classify_wrapper('RANDOMFOREST', training_set(:,fea) , testing_set2(:,fea), training_labels(:), ...
                    testing_labels(:), options);
                [temp_stats3_sw,~] = Classify_wrapper('RANDOMFOREST', training_set(:,fea) , testing_set3(:,fea), training_labels(:), ...
                    testing_labels(:), options);
        ...
        save(['CT_TI_3Dresampled_train_grp1_',num2str(num_top_feats),'_',convertStringsToChars(pruning_level),'_Randomforest.mat'],...
            'fea_cell','top_fea','fea_store','HO_AUC','HO_AUC1','HO_AUC2','HO_AUC3','stats1','stats2','stats3');
    end
