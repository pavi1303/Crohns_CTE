...
b=1; %bandwidth
    %for l = [1,1.4,1.7,2]
    l=[.88,1.4142,2.0750,2.828];
    for l=[.7653,1.2755,1.7857,2.2959,2.8061] %lambda = wavelength
    %for l=[1.8:0.43:9]
    %l=1.5;
        %for xy = (0:3)*pi/4
        ...
        %xy=0;
            g = g+1;
            [gb_c,gb_s, ws] = gabor2dkerns(xy,l,b,0,1);
            gabor_data(g).cos = gb_c;
            gabor_data(g).sin = gb_s;
            gabor_data(g).title = sprintf('Gabor XY-%c=%.3f, %c=%.3f, BW=%.1d',952,xy,955,l,b);
            gabor_data(g).ws = ws;
            gabor_data(g).params = [xy,l,b];
